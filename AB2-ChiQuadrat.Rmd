...
Ein Forscherteam untersucht die Wirksamkeit einer Anti-Raucher-Kampagne. Dazu wird das Rauchverhalten (ja/nein ) in einer Stichprobe zweimal untersucht: einmal vor der Kampagne und einmal nach der Kampagne. Die Vermutung ist, dass die Rauchkampagne wirksam ist. Untersuchen Sie dies mit einer gerichteten Hypothese.
...
```{r}
csq <- generate_chisq(labels.a = c("Ja (vorher)"," Nein (vorher)"),labels.b=c("Ja (Nachher)","Nein (Nachher)"),
...
`r chisq_crit(csq, undirected=FALSE)`

Als Effektstärke können wir das Verhältnis der Wechsler berechnen:
$`r mcnemar_effectsize(csq,1)`$

oder als Prozent der Wechsler von Raucher zu Nichtraucher:
$`r mcnemar_effectsize(csq,2)`$
