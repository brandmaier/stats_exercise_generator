...
expected <- c(15.7,10.7,13,60.6)*n/100
...
In einer (fiktiven) Studie wurde untersucht, ob sich die Fehlzeiten von Studierenden über die Semester hinweg unterscheiden. Folgende Daten wurden erhoben:

```{r}
csq <- generate_chisq(labels.b = c("SS19","WS19/20","SS20","SS20/21"), labels.a=c("ja","nein"),tbl = matrix(c(88,12,95,5, 93,7,95,5), ncol=5))
...
```{r}
tbl <- cbind(rowSums(csq$tbl[,1:4]),csq$tbl[,5])
csq <- generate_chisq(labels.b = c("Mo bis Do","Fr"), labels.a=c("ja","nein"),tbl=tbl)
chisq_data_table(csq, FALSE, FALSE)
