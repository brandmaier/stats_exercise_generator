...
Außerdem ist die mittlere Innerhalb-Quadratsumme bekannt: $MQS_{inn}=`r aov1$mqs_wth`$ mit Freiheitsgraden $df_{inn}=`r aov1$df2`$. In jeder Bedingung wurden `r aov1$nz` Personen beobachtet. Das Signifikanzniveau sei `r print_alpha(alpha)`.
```{r results="asis"}
...
Außerdem ist die mittlere Innerhalb-Quadratsumme bekannt: $MQS_{inn}=`r aov1$mqs_wth`$  mit Freiheitsgraden $df_{inn}=`r aov1$df2`$. In jeder Bedingung wurden `r aov1$nz` Personen beobachtet. Als Signifikanzniveau wurde `r print_alpha(alpha)` festgelegt. 
```{r results="asis"}
...
Außerdem ist die mittlere Innerhalb-Quadratsumme aus der Varianzanalyse bekannt: $MQS_{inn}=`r aov1$mqs_wth`$  mit Freiheitsgraden $df_{inn}=`r aov1$df2`$. In jeder Bedingung wurden `r aov1$nz` Personen beobachtet. Als Signifikanzniveau wurde `r print_alpha(alpha)` festgelegt. 
```{r results="asis"}
cat(" Für den Kontrast gilt (beim gerichteten Testen wird das doppelte Alpha angesetzt): ",f_crit(ctr1))
```
