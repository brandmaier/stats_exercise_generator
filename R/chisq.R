...
#' 
generate_chisq <-
  function(labels.a = c("Antidepressivum", "Placebo"),
           labels.b = c("ja", "nein"),
           tbl = matrix(c(25, 33, 15, 7), ncol = 2),
           alpha = 0.05,
           short.labels.a = NULL,
           short.labels.b = NULL) {
    ...
    list(
      ...
      chisq = chisq,
      df = df,
      short.labels.a = short.labels.a,
      short.labels.b = short.labels.b,
      p = p,
      n = n
    )
  }

chisq_data_table <- function(x,
                             type = "",
                             caption = "",
                             position = "!h",
                             format = NULL) {
  if (type == "") {
    ...
    if (caption == "")
      caption = "Kontingenztabelle unter Unabhängigkeitsannahme"
    ...
    if (caption == "")
      caption = "Kontingenztabelle mit Randsummen"
...
chisq_crit <- function(x, undirected = TRUE) {
  ...
  paste0(
    "Der kritische Wert einer $\\chi^2$-Verteilung mit ",
    df,
    " Freiheitsgraden und $\\alpha=$ ",
    alpha * 100,
    "% ",
    twos,
    " ist ",
...
chisq_decision <- function(x) {
  ...
  paste0(
    "Der empirische Wert der Prüfgröße ist ",
...
#'
generate_chisq_uni <-
  function(obs, exp_relative, labels, alpha = 0.05) {
...
solution_chisq_uni <- function(x, part = 1)
{
  ...
  part2 <- paste0(
    "\\chi^2=",
    paste0g("{\\frac{(", x$obs, "-", x$exp, ")^2}{", x$exp, "}}", collapse =
              "+"),
    "=",
    x$chisq
  )
...
chisq_to_data <- function(x) {
  ...
  for (i in 1:x$nrows) {
    for (j in 1:x$ncols) {
      ...
      laba <- x$labels.a[i]
      labb <- x$labels.b[j]
      if (!is.null(x$short.labels.a))
        laba <-  x$short.labels.a[i]
      if (!is.null(x$short.labels.b))
        labb <-  x$short.labels.b[j]
