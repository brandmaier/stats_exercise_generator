generate_anova <- function(av.name = "",
  ...
  dat <- round( 
    rnorm(n=n*num.reps, rep(grp_means, each=num.obs.per.fac), within)+
      rep(person_icepts, each=num.reps), 
    obs_round)
  dat <- pmax(dat, obs_min)
  dat <- pmin(dat, obs_max)
  dat <- data.frame(av=dat, # dependent variable
                    grps=rep(1:num.facs,each=num.obs.per.fac*num.reps),
                    pids=rep(1:num.obs.per.fac,num.facs,each=num.reps), # person ID
