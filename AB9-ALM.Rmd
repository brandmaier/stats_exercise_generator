...
header-includes:
   ...
   - \fancyhf{}
   - \lhead{Statistik II}
   - \chead{Allgemeines lineares Modell}
...
```
`r start_intro_box()`
Dieses Arbeitsblatt dient dazu, Rechenaufgaben zum Allgemeinen Linearen Modell (ALM) zu üben.

\textbf{Voraussetzungen:}
\begin{itemize}
\item Grundlagen der einfachen und multiplen Regression
\item Verständnis von Dummy-Codierung
\item Grundlagen des Hypothesentestens
\end{itemize}
\textbf{Sie üben heute:}
\begin{itemize}
\item Kategoriale Prädiktoren dummy-codieren
\item Regressionskoeffizienten interpretieren
\item Modelle vergleichen mit dem F-Test
\item Adjustiertes $R^2$ berechnen
\end{itemize}
\end{tcolorbox}
# Aufgabe 1 - Dummy-Codierung

In einer Studie wurde erhoben, mit welchem Verkehrsmittel Studierende zur Uni gelangen. Die vier Antwortmöglichkeiten waren: zu Fuß, Fahrrad, Öffentlich, oder Anders. Damit dieses Merkmal als Prädiktor in einer Regression benutzt werden kann, soll es dummy-codiert werden. Die Referenzkategorie soll "Fahrrad" sein. Erstellen Sie eine Dummy-Codierung und tragen Sie diese in folgende Tabelle ein.

In dieser Tabelle finden Sie die Daten, die für das Merkmal "Verkehrsmittel" erhoben wurden, in der ersten Spalte. Nutzen Sie die übrigen Spalten, um die Dummy-Codierung zu erstellen. Tragen Sie aussagekräftige Namen für die übrigen Spalten ein.
\newpage
```{r, echo=FALSE}
set.seed(4359)
N <- 12
...
if (include_solution)
  solution_header()
knitr::kable(rdisplay, booktabs = FALSE, caption = "Dummy-Codierung des Merkmals Verkehrsmittel")
```
...
knitr::kable(cfs, booktabs=TRUE, caption = "Regressionsergebnisse der Zufriedenheit mit dem Weg zur Universität")
```

Ferner ergab sich $R^2=`r r2 `$ als Modellgütemaß.

a) Wie interpretiert man hier nun Regressionskonstante und -gewichte?
b) Welche Prädiktoren haben einen signifikanten Einfluss?
...
cat("Die Regressionskonstante entspricht dem durchschnittlichen Wert der Referenzkategorie, die hier 'Fahrrad' sein sollte.",
    ...
    paste0("Daraus folgt, dass die Zufriedenheit von Fußgängern ", ests[1], " + (", ests[4], ") = ", (ests[1]+ests[4]), " beträgt."))

cat("\n\nb) ")
cat("Keiner der Prädiktoren hat einen signifikanten Einfluss, da alle p>0.05.")


cat("\n\nc) ")
cat("Adjustiertes Bestimmtheitsmaß berechnen: ")
cat("\\vspace{-1cm}")
```
...
Das obengenannte Modell soll verglichen werden mit einem Regressionsmodell, in das noch zwei weitere Prädiktoren aufgenommen werden. Man möchte prüfen, ob die Vorhersagekraft des Modells verbessert werden kann, indem man die Merkmale Außentemperatur und Luftfeuchtigkeit mitaufnimmt. Dieses alternative Modell hat eine geschätzte Modellgüte von $R^2=`r r22`$. 

Ist das Modell signifikant besser oder sind die Daten plausibel unter der Annahme, dass die Modelle in Wahrheit gleich gut vorhersagen? Führen Sie einen Modellvergleich rechnerisch durch und treffen Sie eine Entscheidung in Bezug auf die Nullhypothese.
...
wrap_math_block("H_0: R^2_u = R^2_e \\text{ (Die zusätzlichen Prädiktoren verbessern die Vorhersage nicht.)}")
wrap_math_block("H_1: R^2_u > R^2_e \\text{ (Die zusätzlichen Prädiktoren verbessern die Vorhersage.)}")
cat("\n\n")
cat("\\vspace{0.5cm}\n")
